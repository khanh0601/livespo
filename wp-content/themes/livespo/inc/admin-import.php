...
function livespo_render_import_page() {
        ...
        if (!empty($_FILES['csv_file']['tmp_name'])) {
            ...
            if (($handle = fopen($file, "r")) !== FALSE) {
                ...
                while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                    $row++;
                    if ($row === 1) continue; // Bỏ qua tiêu đề
    ...
    <div class="wrap">
        ...
        <div style="background: #fff; padding: 25px; border: 1px solid #ccd0d4; max-width: 800px; margin-top: 20px;">
            ...
            <p><a href="<?php echo esc_url($sample_csv_url); ?>" class="button button-secondary" download>📥 Tải về File CSV Mẫu</a></p>
            
            <form method="post" enctype="multipart/form-data" style="margin: 20px 0; padding: 20px; border: 2px dashed #b4b9be; background: #f9f9f9; text-align: center;">
                <?php wp_nonce_field('livespo_import_csv_action'); ?>
                <input type="file" name="csv_file" accept=".csv" required style="margin-bottom: 15px;">
                <br>
                <button type="submit" name="submit_import" class="button button-primary button-hero">Tiến hành Import Dữ liệu</button>
            </form>
            
            <?php if (!empty($import_details)): ?>
            <div style="margin-top: 30px; background: #f0f0f1; padding: 15px; border-left: 4px solid #0073aa; max-height: 400px; overflow-y: auto;">
