...
add_action('init', function () {
	...
	add_rewrite_rule(
		'^en/product/([^/]+)/?$',
		'index.php?post_type=product&name=$matches[1]&lang=en',
		'top'
	);
	...
	add_rewrite_rule(
		'^san-pham/([^/]+)/?$',
		'index.php?post_type=product&name=$matches[1]&lang=vi',
		'top'
	);
});

// 2. Đổi permalink theo ngôn ngữ
add_filter('post_type_link', function ($url, $post) {
	...
	if ($lang === 'en') {
		$url = str_replace('/san-pham/', '/product/', $url);
	} else {
		$url = str_replace('/product/', '/san-pham/', $url);
	}

	return $url;
}, 10, 2);

// 3. Redirect 301 về đúng URL chuẩn
